...
module SuperClip.App.Session.Logic
...
open SuperClip.App.Session.Tasks
        ...
        | Cloud.OnJoin (_req, (Ok token)) ->
            ...
            let auth =
                ...
                |> Option.map (fun auth ->
                    ...
                    runner.Pack.UserProps.Credential.SetValue <| Some auth
                    |> ignore
                    auth
                )
...
let private onStubStatus (status : LinkStatus) : ActorOperate =
    fun runner (model, cmd) ->
        ...
        match status with
        | LinkStatus.Linked ->
            runner.Pack.UserProps.Credential.Value
            |> Option.map (fun auth ->
